...
@section('content')
    <div class="w-full flex justify-center mt-14">
        <form action="{{route('admin.url.update',['id'=>$url->id])}}" method="POST" class="w-[70%]">
            @csrf
            <h1 class="font-bold text-3xl">Edit Link</h1>
            <div class="mt-7">
                <label for="original_url" class="opacity-50 font-bold mt-2">Destination</label>
                <input type="url" name="backend_url" value="{{$url->original_url}}"
            ...
            <div class="mt-7">
                ...
                <input type="text" name=title
                    class="border border-border-color  mt-2 outline-none p-2 rounded-sm w-full" value="{{$url->title}}">
                <span class="text-danger text-red-500">
                    @error('title')
                        {{ $message }}
                    @enderror
            ...
            <div class="flex gap-6 mt-7">
                <button type="submit" class=" bg-[#189cbc] text-white text-base px-5 py-2 rounded-sm">Update</button>
