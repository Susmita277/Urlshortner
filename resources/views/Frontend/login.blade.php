...
@section('content')
    <div class=" flex justify-center items-center w-full bg-body-color">
        <form action="{{route('home.login')}}" method="POST" class="w-[30%] rounded-sm bg-white my-10 h-fit p-8">
            @csrf
            ...
            <div class="mt-4"> <button type="submit"
                ...
                <a href="{{route('home.register')}}" class="mt-2 text-md text-link-color">register</a>
            </div>
        </form>
    </div>
@endsection
